...
對動態成交價功能，保留訂閱請求／確認／失敗、第一筆有效資料、過期／斷線、重新訂閱與資料恢復、解除訂閱，以及會話世代與來源識別。記錄觸發策略或提醒時實際使用的價格、來源時間與規則版本，讓分析者能從操作連到決策與後續委託；畫面更新時間不能替代行情來源時間。

畫面節流與一般tick合併可以減少重繪及日誌量，但不能省略交易決策、資料失效、回報遺失／queue超載或未知外部效果。若不保存完整tick流，明示無法重播所有盤中行情；不要用「稽核完整」宣稱每筆市場資料皆已保存。最後有效價的顯示不代表資料已恢復，更不等於使用者自己成交。
